...
smooth puppet-style motion while keeping joints connected.
"""
)
...
def closest_points_between_masks(mask_a, mask_b):
    ...
    if len(pts_a) > max_points:
        idx = np.linspace(0, len(pts_a) - 1, max_points).astype(int)
        pts_a = pts_a[idx]

    if len(pts_b) > max_points:
        idx = np.linspace(0, len(pts_b) - 1, max_points).astype(int)
        pts_b = pts_b[idx]
...
def detect_parts(image, selected_colors):
    ...
    for part in all_parts:
        ...
        for candidate in all_parts:
            ...
            p1, p2, distance = closest_points_between_masks(
                part["mask"], candidate["mask"]
            )
        ...
        if best_parent is not None:
            ...
            part["attachment_distance"] = best_distance
        else:
            part["parent"] = None
            part["base"] = part["center"]
            part["tip"] = farthest_point(
                component_boundary((part["mask"].astype(np.uint8) * 255)),
                part["center"]
            )
...
# ============================================================
# HIERARCHICAL TRANSFORMATION ENGINE
# ============================================================
...
def get_affine_matrix(angle_deg, tx, ty, pivot):
    ...
    # Translate pivot to origin -> Rotate -> Translate back + local offset
    M = np.array([
...
def prepare_background(original, parts):
    ...
    area_ratio = np.count_nonzero(combined) / float(combined.size)
...
def paste_layer(canvas, layer, layer_mask, x, y):

    h, w = canvas.shape[:2]
    lh, lw = layer.shape[:2]

    x1, y1 = max(0, x), max(0, y)
    x2, y2 = min(w, x + lw), min(h, y + lh)
    ...
    src = layer[ly1:ly2, lx1:lx2]

    alpha = layer_mask[ly1:ly2, lx1:lx2].astype(np.float32) / 255.0
    alpha = alpha[:, :, None]

    result = src.astype(np.float32) * alpha + roi.astype(np.float32) * (1.0 - alpha)
    canvas[y1:y2, x1:x2] = np.clip(result, 0, 255).astype(np.uint8)
...
def motion_for_part(motion, part, normalized_time, intensity):
    ...
    depth_factor = 0.65 + min(depth, 3) * 0.15
    strength = intensity * depth_factor
    ...
    if motion == "Idle Breathing":
        if depth == 0:
            ty = sine * strength * 0.12
            angle = sine * strength * 0.10
        else:
            angle = sine * strength * 0.30

    elif motion == "Wave":
        angle = sine * strength * 0.75
        ...
        if depth == 0:
            ty = abs(math.sin(2 * math.pi * t)) * strength * 0.18
            angle = sine * strength * 0.12
        else:
            phase = 2 * math.pi * t + (math.pi if side < 0 else 0)
            angle = math.sin(phase) * strength * 1.0
            ty = max(0, -math.cos(phase)) * strength * 0.18

    elif motion == "Run":
        if depth == 0:
            ty = abs(math.sin(4 * math.pi * t)) * strength * 0.35
            angle = sine * strength * 0.25
        else:
            phase = 4 * math.pi * t + (math.pi if side < 0 else 0)
            angle = math.sin(phase) * strength * 1.5
            ty = max(0, -math.cos(phase)) * strength * 0.35

    elif motion == "Jump":
        jump = math.sin(math.pi * t)
        ty = -jump * strength * 1.5
        if depth > 0:
            angle = sine * strength * 0.30
        ...
        if depth > 0:
            tx = math.sin(4 * math.pi * t) * strength * 0.35
...
def render_frame(original, background, parts, motion, t, intensity):
    ...
    for part in parts:
        ...
        world_M = world_transforms[part["id"]]
        affine_2x3 = world_M[:2, :]
        ...
        canvas = paste_layer(canvas, warped_part, warped_mask, 0, 0)
...
def build_gif(frames, duration):
    ...
    for frame in frames:
        ...
        pil = Image.fromarray(rgb).convert("P", palette=Image.ADAPTIVE)
        prepared.append(pil)
...
def draw_rig(image, parts):
    ...
    for index, part in enumerate(parts):
        ...
        cv2.circle(preview, base, 9, (0, 0, 255), -1)
        cv2.circle(preview, tip, 7, (0, 255, 0), -1)
        cv2.circle(preview, center, 5, color, -1)

        cv2.putText(preview, str(index + 1), (center[0] + 8, center[1] - 8),
...
if uploaded is not None:
    ...
    try:
        ...
        rotation = st.sidebar.selectbox(
            "Rotate photograph",
            [0, 90, 180, 270],
            format_func=lambda x: f"{x}°" if x != 0 else "Original"
        )
        ...
        with col2:
            st.subheader(f"🦴 Detected Rig — {len(parts)} parts")
            if parts:
                rig_preview = draw_rig(image, parts)
                st.image(cv2.cvtColor(rig_preview, cv2.COLOR_BGR2RGB), width="stretch")
            else:
                st.info("Select one or more colored regions.")

        if parts:
            with st.expander("🔍 Detected Parts"):
                for i, part in enumerate(parts):
                    parent = "ROOT" if part["parent"] is None else str(part["parent"] + 1)
                    st.write(
                        f"**Part {i + 1}** | {part['color_name']} | "
                        f"Area: {part['area']} | Parent: {parent} | Depth: {part['depth']}"
                    )
        ...
        if generate:
            ...
            progress = st.progress(0, text="Preparing puppet rig...")
            background = prepare_background(image, parts)
            ...
            st.success(f"🎉 {motion} animation generated!")
            st.subheader("🎬 Preview")
            st.markdown(gif_html(gif_data), unsafe_allow_html=True)
            ...
            with d2:
                ...
                if mp4_data:
                    ...
                    st.info("MP4 encoding was unavailable. GIF is still ready.")

            with st.expander("🎞️ Animation Frames"):
                frame_indices = np.linspace(0, len(frames) - 1, min(8, len(frames))).astype(int)
                cols = st.columns(len(frame_indices))

                for col, idx in zip(cols, frame_indices):
                    with col:
                        st.image(cv2.cvtColor(frames[idx], cv2.COLOR_BGR2RGB), caption=f"Frame {idx + 1}")
